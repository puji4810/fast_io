...
int main()
{
	...
	{
		fast_io::timer t(u8"fastio_char_digit_to_literal_new");
		std::uint64_t sum{};
		char const *p = begin;
		while (p < end)
		{
	...
	// fast_io char_digit_to_literal
	{
#if defined(FAST_IO_ENABLE_NEW)
#undef FAST_IO_ENABLE_NEW
#endif
#define FAST_IO_ENABLE_NEW 0
		fast_io::timer t(u8"fastio_char_digit_to_literal_old");
		std::uint64_t sum{};
		char const *p = begin;
		while (p < end)
		{
